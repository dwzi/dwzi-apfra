...
$(function () {
	...
	$('#ft_{$datasql_edit_fields[ti].row[i].col[j].field}').val(moment('{$data.{$datasql_edit_fields[ti].row[i].col[j].field}}', 'YYYY-MM-DD HH:mm:ss').format('{if isset($datasql_edit_fields[ti].row[i].col[j].format)}{$datasql_edit_fields[ti].row[i].col[j].format}{else}DD.MM.YYYY HH:mm{/if}'));
	...
	$('#datetimepicker_{$datasql_edit_fields[ti].row[i].col[j].field}').datetimepicker({
		...
		showClose: true,
		format: '{if isset($datasql_edit_fields[ti].row[i].col[j].format)}{$datasql_edit_fields[ti].row[i].col[j].format}{else}DD.MM.YYYY HH:mm{/if}'
	});
	
	$('#datetimepicker_{$datasql_edit_fields[ti].row[i].col[j].field}').on("dp.change",function (e) {
		$('#f_{$datasql_edit_fields[ti].row[i].col[j].field}').val(moment(e.date).format('YYYY-MM-DD HH:mm:ss'));
	});
});
//-->
</script>
